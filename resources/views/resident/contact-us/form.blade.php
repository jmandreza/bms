<form id="contact-us-form" action="{{route('resident.contact-us.store')}}" method="post" class="max-w-xl grid gap-y-4">
    @csrf

    <div>
        <x-input-label for="email-field" value="Email" />
        <x-text-input type="email" id="email-field" name="email" class="block mt-1 w-full" :value="old('email', Auth::user()->email)" />
