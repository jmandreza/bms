<form id="new-resident-form" method="POST" action="{{ route('admin.residents.store') }}" x-data="{activeTab : 1, active: 'border-indigo-400 text-gray-900 focus:outline-none focus:border-indigo-700 transition duration-150 ease-in-out', inactive: 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 focus:outline-none focus:text-gray-700 focus:border-gray-300 transition duration-150 ease-in-out'}" >
    @csrf
    ...
    <div x-show="activeTab === 1" class="mt-6 grid sm:grid-cols-2 md:grid-cols-3 gap-4">
        @include('admin.residents.partials.personal-info')
    </div>

    <div x-cloak x-show="activeTab === 2" class="mt-6 grid sm:grid-cols-2 md:grid-cols-3 gap-4">
        @include('admin.residents.partials.citizenship')
    </div>
...
<script type="module">
    $("#new-resident-form").on('submit', function(e) {
        e.preventDefault();
        ...
        Method.submit({
            ...
            text: ['Saving Resident', 'Save Resident'],
        });
    });
</script>